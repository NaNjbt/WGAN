...
from src.dcgannobn_model import DcgannobnD, DcgannobnG
from src.mlp_model import MlpD, MlpG
...
if __name__ == '__main__':
    ...
    generator_config = {"imageSize": args_opt.imageSize, "nz": nz, "nc": nc, "ngf": ngf,
                        "n_extra_layers": n_extra_layers, "noBN": args_opt.noBN, "mlp_G": args_opt.mlp_G}
    ...
    def init_weight(net):
        for _, cell in net.cells_and_names():
            if isinstance(cell, (nn.Conv2d, nn.Conv2dTranspose)):
                ...
                cell.gamma.set_data(init.initializer(ms.Tensor(np.random.normal(1, 0.02, cell.gamma.shape), ms.float32),
                                                     cell.gamma.shape))
                cell.beta.set_data(init.initializer('zeros', cell.beta.shape))
            elif isinstance(cell, nn.Dense):
                cell.weight.set_data(init.initializer(init.Uniform(math.sqrt(1 / cell.weight.shape[1])), cell.weight.shape))
                cell.bias.set_data(init.initializer(init.Uniform(math.sqrt(1 / cell.weight.shape[1])), cell.bias.shape))
    ...
    if args_opt.noBN:
        ...
        netG = MlpG(args_opt.imageSize, nz, nc, ngf)
    else:
    ...
    if args_opt.mlp_D:
        netD = MlpD(args_opt.imageSize, nz, nc, ndf)
    else:
    ...
    input = ms.Tensor(np.zeros([args_opt.batchSize, 3, args_opt.imageSize, args_opt.imageSize]), dtype=ms.float32)
    noise = ms.Tensor(np.zeros([args_opt.batchSize, nz, 1, 1]), dtype=ms.float32)
    fixed_noise = ms.Tensor(np.random.normal(0, 1, size=[args_opt.batchSize, nz, 1, 1]), dtype=ms.float32)
    ...
    for epoch in range(args_opt.niter):  # niter: the num of epoch
        ...
        while i < length:
            ...
            while j < Diters and i < length:
                ...
                real = data['image']
                noise = ms.Tensor(np.random.normal(0, 1, size=[args_opt.batchSize, nz, 1, 1]), dtype=ms.float32)
