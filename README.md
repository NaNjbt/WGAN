...
- [模型描述](#模型描述)
    - [性能](#性能)
        - [训练性能](#训练性能)
            ...
            - [第三种情况（选用全连接且基础特征数为512的生成器结构）](#第三种情况（选用全连接且基础特征数为512的生成器结构）)
        - [推理性能](#推理性能)
- [随机情况说明](#随机情况说明)
...
WGAN网络包含两部分，生成器网络和判别器网络。判别器网络采用卷积DCGAN的架构，即多层二维卷积相连。生成器网络分别采用卷积DCGAN生成器结构、没有BatchNorm的卷积DCGAN生成器结构以及全连接的基础特征为512的生成器结构。输入数据包括真实图片数据和噪声数据，真实图片resize到64*64，噪声数据随机生成。
...
- Ascend处理器环境运行
  ...
  ```python
  # 运行单机训练示例（包括以下三种情况）：
  sh run_train.sh [DATASET] [DATAROOT] [DEVICE_ID] [NOBN] [MLP_G] [NGF]

  # 第一种情况（选用标准卷积DCGAN的生成器结构）：
  sh run_train.sh [DATASET] [DATAROOT] [DEVICE_ID] False False 64
  
  # 第二种情况（选用没有BatchNorm的卷积DCGAN的生成器结构）：
  sh run_train.sh [DATASET] [DATAROOT] [DEVICE_ID] True False 64

  # 第三种情况（选用全连接且基础特征数为512的生成器结构）：
  sh run_train.sh [DATASET] [DATAROOT] [DEVICE_ID] False True 512
...
在args.py中可以同时配置训练参数、评估参数及模型导出参数。
  ...
  'mlp_G': False, # 是否选用全连接的生成器，默认是不使用
  'mlp_D': False, # 是否选用全连接的判别器，默认是不使用
...
| 参数(M)             | 9.74                                                        |
| 微调检查点 | 26.88M (.ckpt文件)                                         |
| 推理模型        | 26.88M (.mindir文件)                     |
| 脚本                    | [WGAN脚本](https://gitee.com/mindspore/mindspore/tree/r1.2/model_zoo/research/cv/WGAN) |

生成图片效果如下：

![GenSample3](imgs/WGAN_3.png "第三种情况生成的图片样本")
